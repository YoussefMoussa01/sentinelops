...
interface User { id: string; username: string; email: string; is_active: boolean; created_at?: string; updated_at?: string }
...
export const UserDetailPage = () => {
  ...
  const save = async (event: React.FormEvent<HTMLFormElement>) => { event.preventDefault(); if (!userId || !user) return; const form = new FormData(event.currentTarget); try { const response = await usersAPI.updateUser(userId, { email: form.get('email'), is_active: form.get('is_active') === 'true' }); const payload = response as { data?: User }; setUser(payload.data || user); setEditing(false) } catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Unable to update user') } }
