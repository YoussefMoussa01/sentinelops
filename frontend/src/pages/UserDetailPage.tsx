...
const roleTone = (role: string) => ({ SUPER_ADMIN: 'critical', SOC_ADMIN: 'critical', SECURITY_ANALYST: 'investigating', INVESTIGATOR: 'low', VIEWER: 'medium' }[role] || 'medium')

export const UserDetailPage = () => {
  const { userId } = useParams<{ userId: string }>(); const navigate = useNavigate()
  const { user: currentUser } = useAuth(); const canManageSuperAdmin = currentUser?.roles?.includes('SUPER_ADMIN')
